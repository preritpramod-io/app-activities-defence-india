        ...
        <div className="card-glass p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Access password"
                className={`w-full pl-10 pr-4 py-3 bg-navy-800 border rounded-lg text-white placeholder-slate-600 focus:outline-none focus:ring-2 transition-all ${error ? 'border-red-500/60 focus:ring-red-500/30' : 'border-navy-500/40 focus:ring-saffron-500/30 focus:border-saffron-500/50'}`} autoFocus />
            </div>
            {error && <p className="text-red-400 text-sm text-center">Invalid password. Contact the administrator.</p>}
            <button type="submit" className="w-full flex items-center justify-center gap-2 py-3 bg-saffron-500 hover:bg-saffron-600 text-navy-900 font-semibold rounded-lg transition-colors">
              Enter Dashboard <ArrowRight className="w-4 h-4" />
            </button>
          </form>
        </div>
        <p className="mt-6 text-center text-xs text-slate-600">INSPIN Private Limited &middot; Restricted Access</p>
      </div>
      <style>{`
