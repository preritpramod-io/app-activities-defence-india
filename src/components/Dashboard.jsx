        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-3">
              ...
              <span className="font-extrabold text-white tracking-tight">INSPIN <span className="text-saffron-500">DEF</span> IND</span>
            </div>
          ...
          <div className="flex gap-1">
            {tabs.map(tab => { const Icon = tab.icon; return (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.key ? 'border-saffron-500 text-saffron-500' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
